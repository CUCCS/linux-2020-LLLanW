...
echo "vsftpd test file for anonymous user" | sudo tee "${anon_path}/test.txt"

# 修改vsftpd.conf
# 允许匿名下载
sed -i -e "/anonymous_enable=/s/NO/YES/g;/anonymous_enable=/s/#//g" "$conf"
# 不允许匿名上传文件
sed -i -e "/anon_upload_enable=/s/^[#]//g;/anon_upload_enable=/s/YES/NO/g" "$conf"
#允许本地登录
sed -i -e "/local_enable=/s/NO/YES/g;/local_enable=/s/#//g" "$conf"
# 允许更改文件系统
sed -i -e "/write_enable=/s/NO/YES/g;/write_enable=/s/#//g" "$conf"
#允许匿名用户在某些条件下创建新目录
sed -i -e "/anon_mkdir_write_enable=/s/YES/NO/g" /etc/vsftpd.conf
#默认传输日志/var/log/xferlog
sed -i -e "/xferlog_file=/s/#//g" "$conf"
...
ls -la "$u_write_path"
echo "vsftpd test file for the login user" | tee "${u_write_path}/test.txt"
